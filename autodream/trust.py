...
NFR-19: Hash-chained audit log — delegates to autodream.dream.write_audit
...
class AuditRow(BaseModel):
    ...
    Epic 4's flat shape (delegated to autodream.dream.write_audit) — see P7."""
    model_config = ConfigDict(frozen=True)
    op: str
    dream_id: str
    ts: str
...
def append_audit_row(
    ...
    *,
    ...
    DN2 / P7: delegates to autodream.dream.write_audit for the canonical format.
    Backwards-compatible wrapper for Epic 6 callers that used the old API.
    """
    from autodream.dream import write_audit, _resolve_dreams_dir
...
def verify_audit_chain(audit_path: str) -> dict:
    """DN2 / P28: delegate to autodream.dream.verify_audit_chain (the canonical
    implementation). Returns {"valid": bool, "rows_checked": int,
    "first_error": str | None}.
    """
    from autodream.dream import verify_audit_chain as _verify
    p = Path(audit_path)
    if not p.exists():
