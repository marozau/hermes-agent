...
from autodream.memory import _generate_ulid, read_entries
...
def _build_consolidation_prompt(
    ...
    static_parts = [
        ...
        "",
        "Operations: add, update, supersede, expire",
        "",
...
def _run_consolidation_pass(
    ...
    """Story 10.2: run a single consolidation pass via autodream.llm.llm_call.

    Returns (proposals, cost, outcome). P5: outcome is one of:
    "ok" | "empty" | "parse-failed" | "llm-failed".
    """
    try:
        from autodream.llm import llm_call, LLMSpec
    except ImportError:
...
def _check_soul_guardian_carve_out(memory_dir: Optional[str] = None) -> None:
    """Epic 6 P2: delegates to autodream.trust.check_soul_guardian_carveout.
    Raises RuntimeError if the carve-out is violated (mis-config = abort,
    Hard Invariant #5 / FR-43)."""
    from autodream.trust import check_soul_guardian_carveout
...
def create_dream_artifact(
    ...
    *,
    ...
    from autodream.trust import run_attestation_preflight
    ...
    def _do_create() -> str:
        ...
        if effective_n >= 1 and not proposals and not dry_run:
            ...
            category_weights = None
            try:
                from autodream.memory import build_hit_rate_report, propose_category_weight_nudges
            ...
            for pass_idx in range(effective_n):
                ...
                if not raw_ctx and memory_dir:
                    try:
                        from autodream.memory import _resolve_raw_dir
...
def _undo_journal(
    ...
    from autodream.memory import expire_entry  # noqa: F401 — kept for symmetry
    ...
    for je in reversed(entries):
        ...
        from autodream.memory import _resolve_memory_dir as _r
...
def apply_dream(
    ...
    *,
    ...
    from autodream.recall import read_recall_artifact, regression_blocks_apply
    import yaml as _yaml
    ...
    try:
        from autodream.trust import rebaseline_attestation, sign_patches
        try:
        ...
        logger.warning("autodream.trust unavailable: %s", e)
