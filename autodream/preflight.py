...
def enrich_query_with_yake(
    ...
    """
    try:
        from autodream._yake import extract_keywords
...
def _active_embedding_workload() -> tuple[str, str]:
    ...
    """
    try:
        from autodream.llm import load_providers_config
...
def _resolve_entry_timestamp(entry_id: str) -> Optional[float]:
    """P10 / DN5: look up trajectory entry's created_at via autodream.memory."""
    if not entry_id:
        return None
    try:
...
# V1 test file (test_autodream.preflight.py) can import without crashing.
...
def should_run_preflight(
    ...
    *,
    ...
    _st = time.perf_counter()
    if hits:
        from autodream.memory import _resolve_memory_dir as _rmd
