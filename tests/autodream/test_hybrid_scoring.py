...
class TestEmbeddingCache:
    ...
    def test_cache_stores_and_retrieves(self):
        ...
        from autodream.preflight import _cache_embedding, _get_cached_embedding
        vec = [0.1, 0.2, 0.3]
...
class TestHybridScoring:
    ...
    def test_hybrid_with_mock_embeddings(self, tmp_path):
        ...
        with mock.patch("autodream.llm.llm_embed_one", return_value=similar_vec), \
             mock.patch("autodream.preflight._active_embedding_workload", return_value=("deepseek", "deepseek-embed-v2")), \
             mock.patch("autodream.preflight._resolve_sidecar_path", side_effect=lambda eid, p, m, memory_dir=None: tmp_path / f"{eid}.deepseek-deepseek-embed-v2.vec"):
    ...
    def test_disabled_via_config(self):
        ...
        with mock.patch("autodream.llm.llm_embed_one", side_effect=counting_embed):
...
class TestLatencyBudget:
    ...
    def test_config_flag_default_true(self, tmp_path):
        ...
        with open(str(sidecar), "wb") as f:
            f.write(struct.pack(f"{len(vec)}f", *vec))
        with mock.patch("autodream.llm.llm_embed_one", return_value=vec), \
             mock.patch("autodream.preflight._active_embedding_workload", return_value=("deepseek", "deepseek-embed-v2")), \
             mock.patch("autodream.preflight._resolve_sidecar_path", side_effect=lambda eid, p, m, memory_dir=None: tmp_path / f"{eid}.deepseek-deepseek-embed-v2.vec"):
    ...
    def test_config_flag_false(self):
        ...
        called = []
        with mock.patch("autodream.preflight._get_embedding", side_effect=lambda t, w="": called.append(t) or None):
