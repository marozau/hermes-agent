...
class TestBatchedLlmEmbed:
    ...
    def test_llm_embed_batch_returns_list(self):
        ...
        from autodream.llm import llm_embed

        mock_vectors = [[0.1] * 8, [0.2] * 8]
        with mock.patch("autodream.llm._EMBEDDING_DISPATCH", {"test": lambda prov, texts: mock_vectors}):
    ...
    def test_llm_embed_one_shim(self):
        ...
        from autodream.llm import llm_embed_one

        mock_vector = [0.5] * 8
        with mock.patch("autodream.llm.llm_embed", return_value=[mock_vector]):
    ...
    def test_llm_embed_str_back_compat(self):
        ...
        mock_vector = [0.3] * 8
        with mock.patch("autodream.llm._EMBEDDING_DISPATCH", {"test": lambda prov, texts: [mock_vector]}):
            config = {"recall_embed": type("W", (), {
                "primary": type("P", (), {"provider": "test", "model": "m", "max_tokens": 0, "timeout": 3})(),
                "fallback": [],
                "same_provider_ok": False,
...
class TestSidecarWrite:
    ...
    def test_trajectory_entry_queues_sidecar(self, tmp_path):
        ...
        with mock.patch("autodream.memory._queue_embedding_write") as mock_queue:
            entry_id = add_entry(
                type="trajectory",
    ...
    def test_sidecar_filename_format(self, tmp_path):
        ...
        from autodream.memory import _compute_and_write_sidecar
        ...
        mock_vec = [0.1] * 8
        with mock.patch("autodream.memory._compute_and_write_sidecar") as mock_compute:
...
class TestSidecarReader:
    ...
    def test_apply_hybrid_scoring_disabled(self):
        ...
        from autodream.preflight import apply_hybrid_scoring, TrajectoryHit
    ...
    def test_apply_hybrid_scoring_failed_query(self):
        ...
        hits = [TrajectoryHit(id="e1", entry_id="e1", content="test", bm25_score=1.0)]
        with mock.patch("autodream.llm.llm_embed_one", return_value=None):
    ...
    def test_apply_hybrid_scoring_reads_sidecar(self, tmp_path):
        ...
        with mock.patch("autodream.llm.llm_embed_one", return_value=query_vec), \
             mock.patch("autodream.preflight._resolve_sidecar_path", return_value=sidecar), \
             mock.patch("autodream.preflight._active_embedding_workload", return_value=("deepseek", "deepseek-embed-v2")):
            result, source = apply_hybrid_scoring(hits, "query")
    ...
    def test_apply_hybrid_scoring_missing_sidecar_fallback(self):
        ...
        with mock.patch("autodream.llm.llm_embed_one", return_value=query_vec), \
             mock.patch("autodream.preflight._resolve_sidecar_path", return_value=nonexistent):
            result, source = apply_hybrid_scoring(hits, "query")
...
class TestPerStageInstrumentation:
    ...
    def test_stage_timings_field_exists(self):
        ...
        from autodream.preflight import PreflightTelemetry
...
class TestSloRegression:
    ...
    def test_preflight_latency_budget(self):
        ...
        for _ in range(20):  # 20 iterations for statistical significance
            ...
            with mock.patch("autodream.preflight.classify_intent") as mock_ci, \
                 mock.patch("autodream.preflight.evaluate_skip_ladder") as mock_sl, \
                 mock.patch("autodream.preflight.retrieve_trajectories", return_value=[]), \
                 mock.patch("autodream.preflight._filter_stale_trajectories", return_value=[]), \
                 mock.patch("autodream.preflight.apply_hybrid_scoring", return_value=([], "ok")), \
                 mock.patch("autodream.preflight.rank_trajectories", return_value=[]), \
                 mock.patch("autodream.preflight.dedupe_and_cap", return_value=[]), \
                 mock.patch("autodream.preflight.format_heads_up", return_value="test"), \
                 mock.patch("autodream.preflight.write_preflight_telemetry"), \
                 mock.patch("autodream.preflight.persist_citations"), \
                 mock.patch("autodream.preflight.get_or_create_gate") as mock_gate, \
                 mock.patch("autodream.preflight._load_config", return_value={}), \
                 mock.patch("autodream.preflight._resolve_mode", return_value="live"):
