"""End-to-end smoke tests for hermes_providers registration (Story 3.8)."""
...
class TestRegistration:
    ...
    def test_register_all_wires_everything(self):
        ...
        # Clean slate
        hermes_llm._PROVIDER_DISPATCH.clear()
        ...
        for name in ("anthropic", "deepseek", "openai"):
            assert callable(hermes_llm._PROVIDER_DISPATCH[name]), (
    ...
    def test_register_all_idempotent(self):
        ...
        hermes_llm._PROVIDER_DISPATCH.clear()
        register_all()
        snapshot = dict(hermes_llm._PROVIDER_DISPATCH)

        register_all()

        assert hermes_llm._PROVIDER_DISPATCH == snapshot
...
class TestDispatchEndpoint:
    ...
    def test_dispatch_to_anthropic(self, sample_messages):
        ...
        provider = hermes_llm.ProviderSpec(
            provider="anthropic",
        ...
        with patch("hermes_providers_anthropic.httpx.Client") as mock_client:
            ...
            try:
                result = hermes_llm._call_provider_api(provider, sample_messages)
    ...
    def test_dispatch_unregistered(self, sample_messages):
        ...
        with pytest.raises(NotImplementedError, match="no dispatcher.*nonexistent"):
            hermes_llm._call_provider_api(provider, sample_messages)
...
class TestLLMCallIntegration:
    ...
    def test_llm_call_fatal_on_no_dispatcher(self):
        ...
        primary = hermes_llm.ProviderSpec(
            provider="nonexistent",
            model="fake",
            max_tokens=100,
            timeout=5,
        ...
        workload = hermes_llm.WorkloadSpec(
            primary=primary,
            fallback=[],
            cache="none",
        )
        config = {"test_workload": workload}
        spec = hermes_llm.LLMSpec(
            workload="test_workload",
            messages=[{"role": "user", "content": "hi"}],
        )

        with pytest.raises(NotImplementedError):
            hermes_llm.llm_call(spec, providers_config=config)
