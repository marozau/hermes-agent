...
def provider_spec():
    ...
    Uses ProviderSpec from hermes_llm so tests match real usage.
    """
...
class TestDispatcherRegistration:
    ...
    def test_register_all_wires_anthropic(self):
        ...
        if "anthropic" in hermes_llm._PROVIDER_DISPATCH:
            del hermes_llm._PROVIDER_DISPATCH["anthropic"]

        register_all()
        assert "anthropic" in hermes_llm._PROVIDER_DISPATCH
        assert callable(hermes_llm._PROVIDER_DISPATCH["anthropic"])

    def test_register_all_wires_deepseek(self):
        import hermes_llm
        from hermes_providers import register_all

        if "deepseek" in hermes_llm._PROVIDER_DISPATCH:
            del hermes_llm._PROVIDER_DISPATCH["deepseek"]

        register_all()
        assert "deepseek" in hermes_llm._PROVIDER_DISPATCH
        assert callable(hermes_llm._PROVIDER_DISPATCH["deepseek"])

    def test_register_all_wires_openai(self):
        import hermes_llm
        from hermes_providers import register_all

        if "openai" in hermes_llm._PROVIDER_DISPATCH:
            del hermes_llm._PROVIDER_DISPATCH["openai"]

        register_all()
        assert "openai" in hermes_llm._PROVIDER_DISPATCH
        assert callable(hermes_llm._PROVIDER_DISPATCH["openai"])
    ...
    def test_register_all_idempotent(self):
        ...
        register_all()
        count_before = len(hermes_llm._PROVIDER_DISPATCH)
        register_all()
        count_after = len(hermes_llm._PROVIDER_DISPATCH)
...
class TestAnthropicViaDispatcher:
    ...
    def test_anthropic_chat_via_dispatcher(self, messages):
        ...
        from hermes_llm import _PROVIDER_DISPATCH, ProviderSpec
