...
Covers FR-41..FR-44, NFR-15, NFR-19, plus the code-review patches:
  ...
  DN2  single canonical audit format (delegates to hermes_dream.write_audit)
...
# ─────────────────────────────────────────────────────────────────────────────
# Audit chain — DN2 / P7: delegates to hermes_dream.write_audit
# ─────────────────────────────────────────────────────────────────────────────


class TestAuditChainDelegation:
    def test_append_delegates_and_verifies(self, tmp_path, monkeypatch):
        """One canonical format: append_audit_row writes via hermes_dream.write_audit."""
